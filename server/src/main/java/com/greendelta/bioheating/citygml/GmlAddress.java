...
public record GmlAddress(
	...
	static GmlAddress of(Building b) {
		...
		for (var prop : b.getAddresses()) {
			...
			if (obj == null || obj.getXALAddress() == null) continue;
	...
	private static GmlAddress ofXal(Address a) {
		...
		var country =
			a.getCountry() != null ? strOf(a.getCountry().getNameElements()) : null;
		var locality =
			a.getLocality() != null ? strOf(a.getLocality().getNameElements()) : null;
		var postalCode = postalCodeOf(a);
		var number = numberOf(a);

		return new GmlAddress(country, locality, postalCode, street, number);
	}
	...
	private static String postalCodeOf(Address a) {
		...
		for (var id : code.getIdentifiers()) {
			if (id != null) return id.getContent();
		}
		return null;
	}
